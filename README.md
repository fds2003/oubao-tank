...
| 护送装甲车 | `5` | 护送运输车到撤离点获胜 |
| 车长同乘 | `6` | 1车2人分工：P1驾驶走位，P2鼠标360°瞄准开火 |
## 操作方式

| | 玩家1 (键盘 / 手柄1) | 玩家2 (键盘 / 鼠标 / 手柄2) |
|---|---|---|
| 移动 | `W` `A` `S` `D` / 左摇杆 / 十字键 | `↑` `←` `↓` `→` / 左摇杆 (普通双人) |
| 瞄准 | 车体朝向 / 右摇杆 360° | 鼠标移动 / 右摇杆 360° |
| 开火 | `F` / `空格` / 手柄 `A` `X` `RT` `RB` | 鼠标左键 / `L` / `回车` / 手柄 `A` `X` `RT` |
| 暂停 | `P` / `Esc` / 手柄 `Start` | `P` / `Esc` / 手柄 `Start` |
| 静音 | `M` | `M` |

> 🎮 **手柄原生支持**：即插即用（支持 Xbox、PS4/PS5、国产 USB 手柄），支持开火后坐力微震与受击强震。

## 游戏设置（主菜单）

- `←` `→` / 手柄十字键：选择地图（12张）
- `C` / `V` / 手柄 `LB` `RB`：切换车型（轻/中/重/突击炮）
- `Q` / 手柄 `Y`：切换AI难度（简单/普通/困难）
- `Z` `X` / 手柄 `X`：调整AI数量（1-10）
- `回车` / `空格` / 手柄 `A` `Start`：开始游戏
- `1` ~ `6` / 鼠标点击胶囊按钮：直选 6 大游戏模式
## 游戏规则
