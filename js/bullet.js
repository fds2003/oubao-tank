...
class Bullet {
  ...
  update(dt, game) {
    ...
    // 碰撞检测管道（按优先级依次执行，发生碰撞立即终止）
    if (this._checkHQHit(game)) return;
    if (this._checkTerrainHit(game)) return;
    if (this._checkTankHit(game)) return;
    if (this._checkTransportHit(game)) return;
    this._checkBulletHit(game);
  }

  /** 1. 基地保卫战基地碰撞判定 */
  _checkHQHit(game) {
    if (game.gameMode !== 3 || !game.baseDefense) return false;
  ...
  /** 2. 地形障碍物（边界/钢墙/砖墙）碰撞判定 */
  _checkTerrainHit(game) {
    ...
    if (t === 'B') {
      ...
      game.addShake(1.5);
      // 突击炮特性：击碎砖墙后穿透继续飞行
      if (this.owner.pierce) return true;
  ...
  _checkTankHit(game) {
    for (const tk of game.tanks) {
      if (tk === this.owner || !tk.alive || tk.team === this.owner.team) continue;
      ...
      if (Math.abs(this.x - tk.x) < CONFIG.BULLET_HIT_RANGE && Math.abs(this.y - tk.y) < CONFIG.BULLET_HIT_RANGE) {
        ...
        if (phys.isRicochet) {
          ...
          const refl = game.physics.getRicochetDir(this.dir, { x: nx / nl, y: ny / nl });
          this.dir = refl;
  ...
  /** 4. 护送装甲车运输车碰撞判定 */
  _checkTransportHit(game) {
    if (game.gameMode !== 4 || !game.convoyEscort || !game.convoyEscort.transport.alive || this.owner.team !== 1) return false;
