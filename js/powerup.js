...
// 表驱动矢量图标绘制表（消除 90+ 行 if-else 阶梯，符合 OCP 开闭原则）
const ICON_DRAWERS = {
  shield(ctx, s) {
    ctx.beginPath();
    for (let i = 0; i < 6; i++) {
...
function drawPowerIcon(ctx, type, s) {
  ...
  const drawer = ICON_DRAWERS[type];
...
// 遍历敌方存活坦克的辅助函数（消除重复过滤逻辑）
function getLivingEnemies(game, sourceTank) {
  const enemies = [];
  for (const t of game.tanks) {
    if (t !== sourceTank && t.alive && t.team !== sourceTank.team) {
      enemies.push(t);
    }
  }
  return enemies;
...
function applyPower(game, tank, type) {
  ...
  if (type === 'freeze') {
    const enemies = getLivingEnemies(game, tank);
    const dur = enemies.length > 0 ? Math.max(0.6, 2.2 / Math.sqrt(enemies.length)) : 0;
    for (const t of enemies) t.buff.freeze = dur;
    AudioSys.freeze();
    game.parts.text(tank.x, tank.y - 30, enemies.length > 1 ? '全屏冰冻！' : '冰冻！', enemies.length > 1 ? '#9ad8ff' : info.color);
    return;
  }
  if (type === 'heal') {
    tank.hp = Math.min(tank.maxHp || HP_MAX, tank.hp + CONFIG.HEAL_AMOUNT);
    AudioSys.pickup();
  ...
  if (type === 'mega') {
    tank.hp = Math.min((tank.maxHp || HP_MAX) + CONFIG.MEGA_HP_BONUS, tank.hp + CONFIG.MEGA_HP_BONUS);
  ...
  if (type === 'emp') {
    ...
    for (const t of enemies) {
      if (t.buff.shield > 0) { t.buff.shield = 0; broken++; }
      t.buff.speed = 0;
    ...
    game.parts.text(tank.x, tank.y - 30, broken > 0 ? '护盾摧毁+减速！' : 'EMP减速！', info.color);
    return;
  }
